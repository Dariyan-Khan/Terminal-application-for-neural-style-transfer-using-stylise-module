...
def build_parser():
    ...
    parser.change_output('--change_output', action='store_true', dest='change_output',
                         help='change output to be generated based on content and style images')
    return parser
...
def get_char_index(filename,char):
    return int(filename.find(char))
...
def main():
    ...
    # content_prefix = options.content[get_char_index(options.content,'/')+1:get_char_index(options.content,'.')]
    # first_style=options.styles[0]
    # print(type(options.styles))
    # assert type(options.styles) =='list'
    # style_prefix = first_style[get_char_index(first_style,'/')+1:get_char_index(options.content,'.')]
    # output_prefix = options.output[0:get_char_index(options.output,'/')+1]
    # options.output = output_prefix + content_prefix + '_' + style_prefix +  '.jpg'
    ...
    for content_image_choice in content_images:
        for style_image_choice in style_images:
            for iteration, image, loss_vals in stylize(
                network=options.network,
                initial=initial,
                initial_noiseblend=options.initial_noiseblend,
                content=content_image_choice,
                styles=style_image_choice,
            ...
            if options.change_output:
                content_prefix = content_image_choice[get_char_index(content_image_choice, '/') + 1:get_char_index(content_image_choice, '.')]
                #first_style = options.styles[0]
                #print(type(options.styles))
                #assert type(options.styles) == 'list'
                style_prefix = style_image_choice[get_char_index(style_image_choice, '/') + 1:get_char_index(style_image_choice, '.')]
                output_prefix = options.output[0:get_char_index(options.output, '/') + 1]
                options.output = output_prefix + content_prefix + '_' + style_prefix + '.jpg'
